...
static uint64_t *barrier_flag;
static void *baseaddr;

static int sense;
static int barrier_init = 0;
static int barrier_seq;
...
static int Init_shm_barrier_init(int is_root)
{
    ...
    barrier_flag = (uint64_t *) (memory.base_addr + MPIDU_SHM_CACHE_LINE_LEN + sizeof(MPIDU_Init_shm_block_t) * num_ranks);
    if (is_root) {
        cxl_nt_store_uint64(&barrier->val, 0);
        cxl_nt_store_uint64(&barrier->wait, 0);
        for (int i = 0 ; i < num_ranks; i++) {
            cxl_nt_store_uint64(&barrier_flag[i], 0);
        }
        // MPL_atomic_store_int(&barrier->val, 0);
        // MPL_atomic_store_int(&barrier->wait, 0);
    }
    clflush_region_with_mfence(barrier, sizeof(Init_shm_barrier_t));
    sense = 0;
    barrier_init = 1;
    barrier_seq = 0;
...
static int Init_shm_barrier(void)
{
    ...
    barrier_seq++;
    cxl_nt_store_uint64(&barrier_flag[my_rank], barrier_seq);

    // struct timespec start, current;
    // clock_gettime(CLOCK_MONOTONIC, &start);

    // fprintf(stderr, "Init_shm_barrier cnt %d  enter barrier rank %d\n", barrier_seq, my_rank);
    int completed = 1;
    do {
        completed = 1;
        for (int i = 0 ; i < num_ranks; i ++) {
            if (cxl_nt_load_uint64(&barrier_flag[i]) < barrier_seq) {
                completed = 0;
                break;
            }
        }
        ...
        //     (current.tv_nsec - start.tv_nsec) > 6000000000) {
        //     if (my_rank == 0) {
        //     fprintf(stderr, "Init_shm_barrier cnt %d  rank %d flags:", barrier_seq, my_rank);
        //     for (int i = 0 ; i < num_ranks; i ++) {
        //         fprintf(stderr, "rank %d %d", i, cxl_nt_load_uint64(&barrier_flag[i]));
        //         fprintf(stderr, "\n");
        //     }
        //     }
        // }
    ...
    // fprintf(stderr, "Init_shm_barrier cnt %d  release barrier rank %d\n", barrier_seq, my_rank);
...
int MPIDU_Init_shm_init(void)
{
    ...
    MPIR_FUNC_ENTER;
    #ifdef MPL_USE_CXL_SHM
    global_size = MPIR_Process.size;
    my_global_rank = MPIR_Process.rank;
    num_ranks = global_size;
    my_rank = my_global_rank;
    #else
    local_size = MPIR_Process.local_size;
    my_local_rank = MPIR_Process.local_rank;
    num_ranks = local_size;
    my_rank = my_local_rank;
    #endif

    #ifdef MPL_USE_CXL_SHM
    cxl_shm_init(num_ranks, my_rank);
    srand(getpid());
    #endif


    size_t segment_len = MPIDU_SHM_CACHE_LINE_LEN + sizeof(MPIDU_Init_shm_block_t) * num_ranks + sizeof(uint64_t) * num_ranks;
     ...
     */
    MPIR_pmi_bcast(serialized_hnd, serialized_hnd_size, MPIR_PMI_DOMAIN_ALL);
    if (num_ranks != 1) {
        MPIR_Assert(num_ranks > 1);
        if (my_rank > 0) {
...
int MPIDU_Init_shm_put(void *orig, size_t len)
{
    ...
    MPIR_Memcpy((char *) baseaddr + my_rank * sizeof(MPIDU_Init_shm_block_t), orig, len);
    clflush_region_with_mfence((char *) baseaddr + my_rank * sizeof(MPIDU_Init_shm_block_t), len);
...
int MPIDU_Init_shm_get(int rank, size_t len, void *target)
{
    ...
    MPIR_Assert(rank < num_ranks && len <= sizeof(MPIDU_Init_shm_block_t));
    clflush_region_with_mfence((char *) baseaddr + rank * sizeof(MPIDU_Init_shm_block_t), len);
    MPIR_Memcpy(target, (char *) baseaddr + rank * sizeof(MPIDU_Init_shm_block_t), len);
...
int MPIDU_Init_shm_query(int rank, void **target_addr)
{
    ...
    MPIR_Assert(rank < num_ranks);
    *target_addr = (char *) baseaddr + rank * sizeof(MPIDU_Init_shm_block_t);
    clflush_region_with_mfence((char *) baseaddr + rank * sizeof(MPIDU_Init_shm_block_t), sizeof(MPIDU_Init_shm_block_t));
